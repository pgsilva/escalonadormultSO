...
public class Memoria {
		...
		for (int i = 1; i <= TAMANHO_FIXO_MEMORIA; i++) {
			...
			for (Processo p : processos) { // verifica se alguns dos processos chegou em algum momento e aloca no
				...
				if (p.getTempoChegada() == i) {
					...
					if (
							(this.particoes.get(0).getTamanhoAtual() - p.getDuracaoUU()>this.particoes.get(1).getTamanhoAtual() - p.getDuracaoUU())
						...
						if (this.particoes.get(0).validaEspaco(p) == null) {
							...
							deuErroReserva();
								...
								(this.particoes.get(1).getTamanhoAtual() - p.getDuracaoUU() > this.particoes.get(0).getTamanhoAtual() - p.getDuracaoUU()) 
						...
						if (this.particoes.get(1).validaEspaco(p) == null) {
							...
							(this.particoes.get(2).getTamanhoAtual() - p.getDuracaoUU() > this.particoes.get(1).getTamanhoAtual() - p.getDuracaoUU())
							&& (this.particoes.get(2).getTamanhoAtual() - p.getDuracaoUU() > this.particoes.get(0).getTamanhoAtual() - p.getDuracaoUU())
						...
						if (this.particoes.get(2).validaEspaco(p) == null) {
							...
							(this.particoes.get(3).getTamanhoAtual() - p.getDuracaoUU() > this.particoes.get(1).getTamanhoAtual() - p.getDuracaoUU())
							&& (this.particoes.get(3).getTamanhoAtual() - p.getDuracaoUU() > this.particoes.get(2).getTamanhoAtual() - p.getDuracaoUU())
	...
	private static void deuErroReserva() {
		System.out.println("Alocacao de memoria cheia no Worst Fit, processo encaminhado para a fila de espera!"); // msg ERRO
	}
